-- Regla de negocio: solo una respuesta puede quedar valorada por necesidad.
-- Ademas actualiza la funcion para devolver un error explicito util para diagnostico.

create unique index if not exists responses_one_feedback_per_post_idx
on public.responses (post_id)
...
begin
    ...
    if exists (
        ...
        where r2.post_id = v_response.post_id
          and r2.feedback_type is not null
          and r2.id <> v_response.id
    ) then
        raise exception 'post_feedback_already_chosen';
    end if;
