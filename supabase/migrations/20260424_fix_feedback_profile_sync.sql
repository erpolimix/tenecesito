-- Fix de robustez para feedback gamificado.
-- Evita fallos por claves foraneas cuando no existe fila en public.profiles.

insert into public.profiles (id)
select distinct src.user_id
from (
    select p.author_id as user_id from public.posts p where p.author_id is not null
    union
    select r.author_id as user_id from public.responses r where r.author_id is not null
) as src
